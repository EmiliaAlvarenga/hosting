...
#Abrimos la conexion con la base de datos del usuario
        ...
        changepass = "update usuarios set password = PASSWORD('%s')where username='%s';" % (newpasswd,usuario)
...
#Abrimos la conexion con la base de datos de mysql
        conexion2 = MySQLdb.connect(host="localhost", user="root", passwd="root", db="mysql")
        cursor2 = conexion2.cursor()
        changepass2 = "set password for %s@localhost = PASSWORD('%s');" % (usuario,newpasswd)
